...
async function handleMsgAwait(sid, bufAll) {
    ...
    if (!rpcMsg.cmd) {
        ...
        let bufEnd = getRpcMsg({ "id": rpcMsg.id }, Buffer.from(JSON.stringify(data)), 7 /* define.Rpc_Msg.rpcMsgAwait */);
...
class rpc_create {
    ...
    loadRemoteMethod() {
        ...
        fs.readdirSync(dirName).forEach(function (serverName) {
            ...
            let remoteDirName = path.join(dirName, serverName, 'net/remote');
            ...
            if (exists) {
                ...
                fs.readdirSync(remoteDirName).forEach(function (fileName) {
                    if (!fileName.endsWith(app.srcType)) {
                        return;
                    }
                    let fileBasename = path.basename(fileName, app.srcType);
    ...
    sendT(cmd, args) {
        ...
        let bufEnd = getRpcMsg({ "cmd": cmd.file_method }, msgBuf, 6 /* define.Rpc_Msg.rpcMsg */);
    ...
    sendCb(sid, cmd, args) {
        ...
        let rpcTimeout = { "id": getRpcId(), "cb": cb, "time": outTime, "await": false };
        let rpcMsg = {
            "cmd": cmd.file_method,
            "id": rpcTimeout.id
        };
        let bufEnd = getRpcMsg(rpcMsg, Buffer.from(JSON.stringify(args)), 6 /* define.Rpc_Msg.rpcMsg */);
    ...
    sendAwait(sid, notify, cmd, args) {
        ...
        let bufEnd = getRpcMsg(rpcMsg, Buffer.from(JSON.stringify(args)), 7 /* define.Rpc_Msg.rpcMsgAwait */);
        sendTo(sid, rpcTimeout, bufEnd);
        return promise;
    }
}
 ...
 */
function sendRpcMsgToSelf(cmd, msgBuf, cb) {
...
function getCallBackFunc(sid, id) {
    return function (...args) {
        let bufEnd = getRpcMsg({ "id": id }, Buffer.from(JSON.stringify(args)), 6 /* define.Rpc_Msg.rpcMsg */);
        sendTo(sid, null, bufEnd);
    };
}
/**
