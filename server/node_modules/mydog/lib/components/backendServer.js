...
class BackendServer {
     ...
     */
    sendMsgByUidSid(cmdName, msg, uidsid) {
        let groups = {};
        let group;
        let one;
        for (one of uidsid) {
        ...
        let app = this.app;
        let msgBuf = app.protoEncode(app.routeConfig[cmdName].sc, msg);
        let sid;
        let buf;
        for (sid in groups) {
     ...
     * The back-end server sends a message to the client
     */
    sendMsgByGroup(cmdName, msg, group) {
        let app = this.app;
        let msgBuf = app.protoEncode(app.routeConfig[cmdName].cs, msg);
        let sid;
        let buf;
        for (sid in group) {
