...
class RpcServerSocket {
    constructor(app, socket) {
        ...
        this.registerTimer = setTimeout(function () {
            app.logger("error" /* loggerLevel.error */, `${meFilename} register timeout, close the rpc socket: ${socket.remoteAddress}`);
            socket.close();
        }, 5000);
    }
    ...
    onRegisterData(data) {
        try {
            ...
            if (type === 1 /* define.Rpc_Msg.register */) {
                ...
                this.app.logger("error" /* loggerLevel.error */, `${meFilename} illegal rpc register, close the rpc socket: ${this.socket.remoteAddress}`);
                this.socket.close();
            }
        }
        catch (e) {
    ...
    onData(data) {
        try {
            ...
            if (type === 4 /* define.Rpc_Msg.clientMsgIn */) {
                ...
                this.app.logger("error" /* loggerLevel.error */, `${meFilename} illegal data type, close rpc client named: ${this.id}`);
                this.socket.close();
            }
        }
        catch (e) {
            this.app.logger("error" /* loggerLevel.error */, e.stack);
        }
    }
    /**
    ...
    onClose() {
        ...
        this.app.logger("error" /* loggerLevel.error */, `${meFilename} a rpc client disconnected: ${this.id}, ${this.socket.remoteAddress}`);
    }
    /**
     * register
     */
    registerHandle(msg) {
        ...
        try {
            ...
            this.app.logger("error" /* loggerLevel.error */, `${meFilename} JSON parse error，close the rpc socket: ${this.socket.remoteAddress}`);
            this.socket.close();
            return;
        }
        if (data.serverToken !== serverToken) {
            this.app.logger("error" /* loggerLevel.error */, `${meFilename} illegal serverToken, close the rpc socket: ${this.socket.remoteAddress}`);
            this.socket.close();
            return;
        }
        if (this.app.rpcPool.getSocket(data.id)) {
            this.app.logger("error" /* loggerLevel.error */, `${meFilename} already has a rpc client named: ${data.id}, close it, ${this.socket.remoteAddress}`);
            this.socket.close();
            return;
        }
        ...
        this.id = data.id;
        this.app.logger("debug" /* loggerLevel.debug */, `${meFilename} get new rpc client named: ${this.id}`);
    ...
    heartbeatHandle() {
        ...
        this.heartbeatTimer = setTimeout(() => {
            this.app.logger("error" /* loggerLevel.error */, `${meFilename} heartBeat time out, close it: ${this.id}`);
            this.socket.close();
        }, heartbeat * 1000 * 2);
    }
    /**
