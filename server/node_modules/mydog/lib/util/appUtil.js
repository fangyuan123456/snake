...
exports.getNoRpcKey = exports.startServer = exports.f_existsSync = exports.f_basename = exports.f_endsWith = exports.defaultConfiguration = void 0;
...
function f_endsWith(str, hz) {
    if (hz == ".js" || hz == ".ts") {
        if (str.endsWith(".js") || str.endsWith(".ts")) {
            return true;
        }
        else {
            return false;
        ...
        return str.endsWith(hz);
    }
}
exports.f_endsWith = f_endsWith;
function f_basename(str, hz) {
    if (hz == ".js" || hz == ".ts") {
        if (str.endsWith(".js")) {
            return path.basename(str, ".js");
        }
        else {
            return path.basename(str, ".ts");
        }
    }
    else {
        return path.basename(str, hz);
    }
}
exports.f_basename = f_basename;
function f_existsSync(str) {
    if (f_endsWith(str, ".js")) {
        let baseName = f_basename(str, ".js");
        return fs.existsSync(baseName + ".js") || fs.existsSync(baseName + ".ts");
    }
    else {
        return fs.existsSync(str);
    }
}
exports.f_existsSync = f_existsSync;
/**
 * Start the server
 * @param app
 */
...
let loadBaseConfig = function (app) {
    loadConfigBaseApp(app, "masterConfig", path.join(define_1.some_config.File_Dir.Config, 'master'));
    loadConfigBaseApp(app, "serversConfig", path.join(define_1.some_config.File_Dir.Config, 'servers'));
    loadConfigBaseApp(app, "routeConfig", path.join(define_1.some_config.File_Dir.Config, 'protoCfg'));
    function loadConfigBaseApp(app, key, val) {
        let env = app.env;
        let originPath = path.join(app.base, val);
        if (f_existsSync(originPath)) {
