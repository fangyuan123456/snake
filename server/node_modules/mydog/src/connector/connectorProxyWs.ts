...
class ClientSocket implements I_clientSocket {
    ...
    private handshake(data: Buffer) {
        let msg: { "md5": string,uid:number,roomId?:number } = null as any;
        try {
        ...
        if(this.connector.app.clients[msg.uid]){
            this.connector.app.callKickUserFunc(msg.uid);
        }
        this.session.bind(msg.uid)
        if(msg.roomId){
            this.session.set({roomId:msg.roomId})
        }
        if (this.sendCache) {
