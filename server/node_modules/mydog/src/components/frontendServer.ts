...
import { f_basename, f_endsWith, f_existsSync } from "mydog/src/util/appUtil";
...
class ClientManager implements I_clientManager {
    ...
    private loadHandler() {
        ...
        let exists = f_existsSync(dirName);
        if (exists) {
            let self = this;
            fs.readdirSync(dirName).forEach(function (filename) {
                if (!f_endsWith(filename,".js")) {
                    return;
                }
                let name = f_basename(filename, '.js');
    ...
    handleMsg(client: I_clientSocket, msgBuf: Buffer) {
        try {
            ...
            this.app.filter.globalBeforeFilter(data, client.session, (hasError) => {
                ...
                let routeArr = this.app.getCmdRouteArr(data.cmd);
                if (this.serverType === routeArr[0]) {
                    let msg = this.app.msgDecode(data.cmd, data.msg);
                    this.app.filter.beforeFilter(data.cmd, msg, client.session, (hasError) => {
                        if (hasError) {
                            return;
                        }
                        this.msgHandler[routeArr[1]][routeArr[2]](msg, client.session, this.callBack(client, this.app.getCmdName(data.cmd)));
                    });
                } else {
                    this.doRemote(data, client.session, routeArr);
                }
            });
        } catch (e: any) {
     ...
     * Callback
     */
    private callBack(client: I_clientSocket, cmdName: string) {
        let self = this;
        return function (msg: any) {
            if (msg === undefined) {
                msg = null;
            }
            let buf = self.app.protoEncode(self.app.routeConfig[cmdName].sc!, msg);
            client.send(buf);
            self.app.filter.afterFilter(self.app.routeConfig[cmdName].sc!, msg, client.session);
        }
    }
