...
class ClientManager implements I_clientManager {
    ...
    handleMsg(client: I_clientSocket, msgBuf: Buffer) {
        try {
            ...
            this.app.filter.globalBeforeFilter(data, client.session, (hasError) => {
                ...
                let routeArr = this.app.cmdConfig[data.cmd].route.split(".");
                if (this.serverType === routeArr[0]) {
                    ...
                    this.app.filter.beforeFilter(data.cmd, msg, client.session, (hasError) => {
                        ...
                        this.msgHandler[routeArr[1]][routeArr[2]](msg, client.session, this.callBack(client, this.app.cmdConfig[data.cmd].name));
                    });
                } else {
    ...
    private callBack(client: I_clientSocket, cmdName: string) {
        ...
        return function (msg: any) {
            ...
            let buf = self.app.protoEncode(self.app.routeConfig[cmdName].res!, msg);
            client.send(buf);
            self.app.filter.afterFilter(self.app.routeConfig[cmdName].res!, msg, client.session);
        }
    }
