...
class rpc_create {
    ...
    loadRemoteMethod() {
        ...
        let exists = appUtil.f_existsSync(dirName);
        if (!exists) {
            return;
        }
        ...
        fs.readdirSync(dirName).forEach(function (serverName) {
            ...
            let exists = appUtil.f_existsSync(remoteDirName);
            if (exists) {
                if (needRpc) {
                    tmp_rpc_obj[serverName] = {};
                }
                fs.readdirSync(remoteDirName).forEach(function (fileName) {
                    if (!appUtil.f_endsWith(fileName,".js")) {
                        return;
                    }
                    let fileBasename = appUtil.f_basename(fileName, '.js');
