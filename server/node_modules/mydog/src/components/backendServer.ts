...
export class BackendServer {
    ...
    handleMsg(id: string, msg: Buffer) {
        ...
        let routeArr = this.app.getCmdRouteArr(cmd);
        let data = this.app.msgDecode(cmd, msg.slice(5 + sessionLen));
        this.app.filter.beforeFilter(cmd, data, session, (hasError) => {
            if (hasError) {
                return;
            }
            this.msgHandler[routeArr[1]][routeArr[2]](data, session, this.callback(id, cmd, session));
        });
    }
    ...
    sendMsgByUidSid(cmdName: string, msg: any, uidsid: { "uid": number, "sid": string }[]) {
        ...
        let app = this.app;
        let msgBuf: Buffer = app.protoEncode(app.routeConfig[cmdName].sc!, msg);
        let sid: string;
        let buf: Buffer;
        for (sid in groups) {
    ...
    sendMsgByGroup(cmdName: string, msg: any, group: { [sid: string]: number[] }) {
        let app = this.app;
        let msgBuf: Buffer = app.protoEncode(app.routeConfig[cmdName].cs!, msg);
        let sid: string;
        let buf: Buffer;
        for (sid in group) {
