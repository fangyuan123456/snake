...
export default class Application extends EventEmitter {
    ...
    srcType:string = path.extname((require.main as any).filename);
    routeConfig: {[key:string]:I_proto} = {};
                                                                  // route.ts
    masterConfig: ServerInfo = {} as ServerInfo;                                             // master.ts
    serversConfig: { [serverType: string]: ServerInfo[] } = {};                              // servers.ts                                                     // route.ts  (split)
    ...
    getCmdRouteArr(cmd:number):string[]{
        for(let i in this.routeConfig){
            let cfg = this.routeConfig[i];
            if(cfg.sc == cmd || cfg.cs == cmd){
                return cfg.route.split(".");
            }
        }
        return [];
    }
    
    getCmdName(cmd:number):string{
        for(let i in this.routeConfig){
            let cfg = this.routeConfig[i];
            if(cfg.sc == cmd || cfg.cs == cmd){
                return i;
    ...
    getCmdMessageName(cmd:number):string{
        for(let i in this.routeConfig){
            let cfg = this.routeConfig[i];
            if(cfg.sc == cmd){
                return "SC_"+i;
     ...
     */
    sendMsgByUidSid(cmdName: string, msg: any, uidsid: { "uid": number, "sid": string }[]) {
        if (msg === undefined) {
            msg = null;
        }
        this.backendServer.sendMsgByUidSid(cmdName, msg, uidsid);
    }

    /**
     * Send a message to the client
     * @param cmdName   cmdName
     * @param msg   messagerouteConfig
     * @param group   { sid : uid[] }
     */
    sendMsgByGroup(cmdName: string, msg: any, group: { [sid: string]: number[] }) {
        if (msg === undefined) {
            msg = null;
        }
        this.backendServer.sendMsgByGroup(cmdName, msg, group);
    }
